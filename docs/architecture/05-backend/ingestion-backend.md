...
refs: docs/research/01-mvp-research/01, docs/research/01-mvp-research/04 §4
---
...
- 제목·요약·키워드는 LLM으로 생성한다.
  ...
  - 프롬프트에 넣는 본문은 앞 5000자(`_MAX_CHARS`)로 제한한다. 슬롯당 컨텍스트(4096) 안에 프롬프트와 출력(512)이 들어가게 한다(docs/lessons/01). 긴 문서의 정밀 요약은 AI 산출물 Summary가 담당한다.
...
- 구현은 토큰 수를 워커 인프로세스 토크나이저로 측정한다. 동봉한 KURE-v1 토크나이저 파일(`KURE_TOKENIZER_PATH`)을 `tokenizers` 라이브러리로 한 번 로드해 재사용하고, 라인을 한 번에 토큰화한 뒤 CPU 작업을 `asyncio.to_thread`로 오프로드한다. llama-server `/tokenize`(`:8081`) HTTP 의존은 제거했다(라인마다 수천 회 왕복이 대형 문서 인제스트를 타임아웃·연결 실패로 무너뜨려서다, docs/lessons/resolved/03).
- 라인 경계에서만 분할해 표 행이 라인 중간에서 잘리지 않게 한다.
- 인프로세스 토큰 수는 서버 토큰화와 미세하게 다를 수 있으나 안전하다. 청크 목표 512토큰은 임베딩 컨텍스트 8192토큰의 1/16이라 여유가 크고, 임베딩 API는 토큰이 아니라 글을 받아 서버가 다시 토큰화하므로 토크나이저 차이는 임베딩 정확도가 아니라 청크 경계만 미세하게 바꾼다.
...
- 이유는 PostgreSQL text 컬럼이 NUL을 저장하지 못해 청크 적재가 실패하기 때문이다(docs/lessons/02).
