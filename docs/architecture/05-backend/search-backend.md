...
- 스트리밍: `/search/ask/stream`은 파싱·임베딩·검색을 먼저 끝낸 뒤 생성 토큰을 `delta` 이벤트로 흘려보내 첫 토큰까지 시간을 줄인다. 인용 재번호와 근거 본문은 전체 답변이 필요하므로 생성 완료 후 `done` 이벤트로 확정한다. 토큰 수 측정·예산은 비스트리밍과 동일하다.
