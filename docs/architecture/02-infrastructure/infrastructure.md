...
refs: docs/research/01-mvp-research/00 §0, docs/research/01-mvp-research/04 §6
---
...
- Mac mini 네이티브(Metal):
```bash
llama-server -m a.x-4.0-light-q4_k_m.gguf -ngl 99 --parallel 4 -c 16384 --port 8080   # 생성(슬롯 4, 슬롯당 4096)
...
- 생성 서버 슬롯 수(`--parallel`)는 인제스트 워커 동시 잡(`max_jobs`)과 맞춘다(현재 둘 다 4). 슬롯당 컨텍스트(`-c`/슬롯 수)가 메타 프롬프트와 출력을 담아야 한다(대량 동시 업로드 사고는 docs/lessons/01).
