...
- 완료 답변의 인용 `[n]`은 `[n](#cite-n)` 링크로 전처리하고, 뷰어가 `a` 렌더를 오버라이드해 클릭 배지로 그린다(클릭 시 출처 이동). 스트리밍 중에는 백엔드가 이미 1부터 재번호해 보내므로 번호는 최종과 같고(search-backend §5), 인용 카드가 아직 없어 `[n]`을 클릭 없는 평문으로 둔다.
