...
- 키워드/의미 결과는 `documentId`로 그룹화해 문서당 1 카드로 렌더한다(헤더=제목·파일명·최고 score·청크 수). 그룹화·정렬(그룹=최고 score, 청크=score desc)은 프론트에서 수행한다(검색 API는 청크 평면 반환).
- 카드 내 청크는 첫 청크만 노출하고 2개 이상이면 "더보기/접기" 토글로 나머지를 펼친다. 청크 카드=chunk_id·score·본문(전체 표시).
- 키워드 모드는 청크 본문에서 질의 일치 부분을 하이라이트한다. 의미 모드는 문서 keywords를 컨셉 해시태그로 표시한다(청크 단위 컨셉은 백엔드 미제공이라 문서 단위로 근사).
