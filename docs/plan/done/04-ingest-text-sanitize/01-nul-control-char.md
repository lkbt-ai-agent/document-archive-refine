...
> 진단과 정제 규칙 근거는 `docs/lessons/02-pdf-nul-byte-chunk-insert.md`(§Cause·§Fix·C0 제어 문자 표)다.
...
## 문서 반영
- [x] E1 아키텍처에 정제 단계를 반영했다 — `docs/architecture/05-backend/ingestion-backend.md`에 §2-6 추출 본문 정제를 추가했다.
- [x] E2 `docs/lessons/02-pdf-nul-byte-chunk-insert.md` `status`를 resolved로 갱신하고 `# Fix`에 적용 표기를 달았다.
