...
overview: 테스트 데이터 크롤러 MVP 스캐폴딩·구현 — 사이트별 추상화로 청약공고 PDF를 sample-datas/에 적재 (design docs/research/02-test-data-crawler/00 전반).
---

> 라이브러리·툴 버전·API 상세는 context7 MCP로 확인 (§5).
> 참조 `(§n)`은 `docs/research/02-test-data-crawler/00-design.md` 섹션이다.
