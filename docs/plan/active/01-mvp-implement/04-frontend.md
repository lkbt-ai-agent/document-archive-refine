...
- [x] D27 표기 순서 정정
  ...
  - 현재 큰=`llmTitle`/작은=`name`를 맞바꾼다(document-frontend 반영). D26과 같은 제목 영역.
...
- [ ] D29 (검토) 세 이름 분리 저장 가능성 — 업로드 당시 원본 파일명(`original_filename`, 불변), 사용자 변경 현재 파일명, ai 논리명(`llm_title`)을 각각 별도 보관할 수 있는지 스키마 검토. 현행은 `original_filename`+`llm_title`뿐이라 "현재 파일명"용 신규 컬럼(예: `display_filename`) 추가 필요 여부 판단(documents-schema·document.md).
- [ ] D30 목록 row 우클릭 → "이름 변경" 컨텍스트 메뉴+다이얼로그로 현재 파일명 수정(낙관·롤백), 원본 파일명·ai 논리명은 보존. D29 결정(신규 컬럼·PATCH 엔드포인트)에 의존.
