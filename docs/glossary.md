...
토큰들 사이의 관계를 한꺼번에 따져(어텐션) 문맥을 파악하는 신경망 구조. 현대 LLM과 임베딩 모델의 공통 뼈대.

- "어텐션"으로 멀리 떨어진 단어끼리의 연관까지 본다.
- 생성 모델(LLM)도 임베딩 모델(KURE-v1)도 이 구조 위에 만들어진다.

### 토큰 (Token)

LLM이 텍스트를 처리하는 최소 단위. 단어 전체일 수도 있고, 단어 일부나 문장부호일 수도 있다.

- 예: "안녕하세요 저는 개발자입니다."를 자르면 `["안녕", "하세요", " 저는", " 개발", "자", "입니다", "."]`가 된다.
- 모델은 문자열 자체가 아니라 토큰 시퀀스를 입력받아 처리한다.
...
긴 문서를 여러 개의 작은 텍스트 조각으로 나누는 과정이 청킹, 그 조각이 청크.

- 보통 토큰 수 기준(예: 500~1000토큰)으로 나눈다.
- 문맥 유지를 위해 인접 조각을 일부 겹치게(오버랩) 만들기도 한다.
...
### 검색 점수 체계 (키워드 score vs 의미 score)

키워드 검색과 의미 검색 모두 결과에 `score`를 붙이지만 의미가 다르다.

- 키워드 score(`pgroonga_score`): 매칭 강도. 질의어가 청크에 얼마나 비중 있게 등장하는지를 재는 빈도 기반(BM25 계열) 랭킹 값이다. 유사도가 아니다.
- 의미 score: 코사인 유사도. DB는 코사인 거리(`distance`)를 돌려주고 앱이 `1 - distance`로 바꾼다. 1에 가까울수록 의미가 가깝다.
- 둘 다 높을수록 그 모드 안에서 상위 후보이지만, "정답"을 보장하진 않는다. 관련도 신호일 뿐이다.
- 계산 방식과 값 범위가 달라 키워드 score와 의미 score를 서로 비교하면 안 된다. 각 모드 안의 정렬에만 쓴다.
...
### 토큰, 토크나이저, 청킹, 청크, 임베딩, 트랜스포머의 관계

여섯 용어는 한 흐름으로 엮인다. 한 줄 정의는 다음과 같다.

- 토큰: 모델이 글을 읽는 최소 조각.
- 토크나이저: 글을 토큰으로 자르는 규칙과 사전.
- 청크: 문서를 나눈 작은 조각.
- 청킹: 문서를 청크로 나누는 과정.
- 트랜스포머: 토큰들의 관계를 계산해 의미를 뽑는 신경망 구조.
- 임베딩: 트랜스포머가 청크의 의미를 담아 낸 숫자 벡터.

처리 흐름은 다음과 같다.

1. 토크나이저가 문서를 토큰으로 자른다.
2. 청킹이 토큰 수를 세어 약 512토큰짜리 청크로 나눈다(64토큰 겹침).
3. 각 청크를 트랜스포머 기반 임베딩 모델에 넣는다.
4. 모델이 청크의 토큰을 처리해 임베딩 벡터를 낸다.
5. 그 벡터를 저장해 의미 검색에 쓴다.

요약하면, 토큰이 기본 단위이고, 토크나이저가 토큰을 만들며, 청킹은 토큰 수로 청크 크기를 맞추고, 트랜스포머는 토큰을 읽어 임베딩을 만드는 엔진이며, 임베딩은 최종 산출물이다.
...
llama-server가 요청을 동시에 처리하는 한 칸. 은행 창구에 비유하면 슬롯 4개는 손님 4명을 동시에 응대하는 창구 4개다.

- 슬롯 수는 `--parallel N`(`-np N`)으로 정한다. 슬롯이 다 차면 새 요청은 대기한다.
- 전체 컨텍스트(`-c`)를 슬롯들이 나눠 갖는다. 예: `-c 8192`를 슬롯 4개가 쓰면 슬롯당 약 2048토큰이다.
- 각 슬롯은 자기 몫의 KV 캐시(대화 기억 공간)를 가진다. 프롬프트 토큰과 출력 토큰의 합이 슬롯당 컨텍스트를 넘으면 "Context size exceeded" 오류가 난다.
- 동시성(슬롯 수)과 슬롯당 크기(`-c` 나누기 슬롯 수)는 트레이드오프다. 슬롯을 늘리면 동시 처리가 늘지만 슬롯당 컨텍스트가 준다.
- 사용 예: `llama-server -m model.gguf --parallel 4 -c 16384`는 슬롯 4개를 두고 각 슬롯에 약 4096토큰을 준다.
