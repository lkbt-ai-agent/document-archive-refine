...
## 기반 (arch 04)
- [ ] A1 모듈 스캐폴드 — `backend/src/{main,config,database,models}` + 도메인 패키지 (§4).
- [ ] A2 설정 — pydantic-settings `.env` 로드, 환경별 분리 (§7).
- [ ] A3 DB 와이어링 — async engine/sessionmaker, `get_session`, `expire_on_commit=False` (§6).
- [ ] A4 공통 규약 — 에러 모델/핸들러, 페이지네이션, CORS, `owner_id` 스코프 강제 (§8).

## 데이터 모델 (arch 03)
- [ ] B1 SQLAlchemy 모델 — Base, folders/documents/document_chunks/계보 (§5·§6).
- [ ] B2 Alembic(`-t async`) 초기화, `version_table_schema='archive'` (§8).
- [ ] B3 수동 마이그레이션 — 확장·ENUM·HNSW·PGroonga 인덱스 (§8).
- [ ] B4 `alembic upgrade head` 원격 적용 + `users` 시드 1명 (§3).
...
- [ ] E4 삭제 수명주기(멱등) + presign TTL 단축(5~15분) (§10).
