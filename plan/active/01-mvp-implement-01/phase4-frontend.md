...
- [ ] 4.7 Right — **DetailInspector(토글형, 문서/폴더 양쪽 §8b)**: 문서=DocumentDetail(status/stage + **"원본 보기"**: 텍스트=MD 뷰어/기타=다운로드, §10) + **MetadataView(읽기 전용 §7a, 업로드·인제스트 소요 시간 + AI 산출물이면 계보 섹션: 부모 문서 링크·프롬프트·모델/seed, `/generations/{id}/lineage`)** + GenerationTrigger + **"산출물 내역"(생성 이력 개명; row 클릭→Center가 산출물 문서 폴더로 이동·선택; 출력 문서 삭제 시 비노출; 생성 소요 시간)**; 폴더=FolderDetail(이름/등록일/하위 항목 수)
