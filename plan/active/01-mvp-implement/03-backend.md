...
- [ ] A5 기동 헬스체크 — PG/MinIO 도달 실패 시 fail-fast, Redis/llama 점검 (§11).
...
- [ ] B3 수동 마이그레이션 — 확장 가용성 확인(`archive_ext`는 02-infra 선설치, `CREATE EXTENSION IF NOT EXISTS` 가드)·ENUM(5종)·HNSW·PGroonga·gin(metadata) 인덱스 (§3·§4).
...
- [ ] C4 `BedrockLLM` 인터페이스 스텁만 — `LLMClient` 준수, 실구현 제외 (system-overview §1, backend §8·§12).

## 폴더 (folders)
- [ ] D1 트리 조회(재귀 CTE 평면 리스트) + CRUD (folders-backend §1·§2).
- [ ] D2 MOVE + 사이클 방지(후손 검증, 422), 형제 중복명 409 (folders.md §5, folders-backend §1).
...
- [ ] E6 문서 조회·이동 API — 목록(`folder_id`·cursor)·상세(`status`/`stage`/`error`)·폴더 이동(`PATCH {folder_id}`) (document-backend §1).
