...
## 1. 개요 / 범위
인접 리스트 기반 폴더 트리의 CRUD·MOVE·재귀 조회/삭제. 소유자 스코프(`owner_id`).
...
- 대안 기각: 머티리얼라이즈드 패스/ltree/네스티드 셋/클로저 테이블은 서브트리 재작성·복잡도로 부적합(research 04 §1).
- 재귀 CTE는 원격 PG 기본 기능(추가 확장 불필요).

## 4. 데이터 모델 참조
`archive.folders`(03 §5): self-FK `parent_id ON DELETE CASCADE`, `uq_folder_sibling_name(parent_id,owner_id,name)`, `ix_folders_parent_id`.

## 5. 트리 조회
- 재귀 CTE로 **소유자 전체 폴더 평면 리스트** 반환 → 프론트에서 `useMemo`로 트리 구성(10).
...
```
검증 통과 시 `UPDATE folders SET parent_id=:new_parent_id WHERE id=:folder_id`. 트랜잭션 내 처리.

## 7. 재귀 삭제
- `ON DELETE CASCADE`로 하위 폴더·문서·청크 연쇄.
- **MinIO 오브젝트는 CASCADE 대상 아님** → service가 삭제 대상 문서의 `object_key`를 먼저 수집 → DB 삭제 후 worker가 오브젝트 삭제(06·03 정합).
...
sequenceDiagram
  ...
  C->>S: PATCH /folders/{id} {parent_id}
  S->>DB: 후손 검증(재귀 CTE)
  alt 대상이 후손
    S-->>C: 422 사이클
  else
    S->>DB: UPDATE parent_id
    S-->>C: 200
  end
```

## 11. 제약·리스크
CASCADE↔MinIO 정리 정합(오브젝트는 앱 책임), 대용량 트리 재귀 CTE 성능.
