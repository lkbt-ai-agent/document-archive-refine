...
- 각 문서는 원본 파일에서 추출한 정보, AI 가 추출한 메타데이터를 함께 보유한다.
  - 서비스에서 생성하는 정보: 등록일시
  - 원본 파일에서 추출한 정보: 원본 파일명, 크기
  - 원본 파일에서 AI가 추출한 메타데이터: 제목, 요약, 토픽, 키워드
- 각 문서의 AI 가 추출한 메타데이터는 MVP에서 사용자 보정 없이 읽기 전용으로 표시한다.
...
- 생성 산출물의 출처로 인용된 문서도 삭제할 수 있으나, 삭제 후에는 해당 출처를 다시 조회할 수 없다(계보 헤드 행은 감사 목적으로 유지, ai-outputs.md §9).
