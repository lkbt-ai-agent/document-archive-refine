...
### 문제
PostgreSQL 기본 `tsvector`/`tsquery`는 언어별 사전 형태소 분석(한국어 문장을 의미 단위로 쪼개는 것: "연봉이"→"연봉"+"이")을 쓰는데 **한국어 사전이 없다**(유럽어 ~15종만). 한국어는 조사·어미가 어간에 붙어 "연봉/연봉이/연봉은/연봉을"이 다른 토큰으로 잡혀 정확 매칭이 조용히 실패한다. `pg_trgm`은 비ASCII 기본 비활성 → 한국어 부적합.
...
> 용어: **N-gram / Bigram** = 텍스트를 n글자씩 잘라 색인하는 기법(Bigram=2글자씩, "연봉"→"연봉","봉이"…), 사전 없이 한국어 부분 일치를 가능케 함. **PGroonga** = PostgreSQL에 고품질 한국어 전문 검색을 더해주는 확장.
...
> 이 절의 핵심 용어: **의미 검색**(질문을 임베딩해 벡터가 가까운 청크를 찾기), **임베딩**(텍스트 의미를 숫자 목록=벡터로 변환), **하이브리드 검색**(키워드+의미 합치기).

### 왜 하이브리드인가
- `pgroonga_score`/`ts_rank`는 **문서 단위 점수(전역 IDF 없음 — IDF=흔한 단어는 덜, 희귀 단어는 더 중요하게 치는 가중치)** → 희귀어 변별 불가. 진짜 BM25(키워드 검색 랭킹 표준 알고리즘)는 TF 포화+IDF+길이정규화.
- 벡터 검색은 의미/패러프레이즈는 잡지만 **정확 토큰(이름·숫자·"연봉")을 놓침**.
- → 둘을 합치는 게 표준.

### 융합 — RRF가 정답
BM25 점수와 cosine(코사인) 점수는 **스케일이 달라 더할 수 없다**. RRF(점수 체계가 다른 두 검색 결과를 순위만으로 합치는 표준 기법)는 점수가 아닌 **순위**를 융합해 스케일 독립:
...
> **RAG**(Retrieval-Augmented Generation) = 관련 문서를 먼저 *검색*해서 그 내용을 LLM에게 근거로 주고 답하게 하는 방식. 아래 파이프라인이 그 검색→답변 흐름의 구체적 단계다.
...
### MVP 파이프라인
> 흐름 한눈에: 쿼리의 의도를 파악(GBNF=출력 형식을 강제하는 문법)해 → 키워드면 바로 검색, 의미/RAG면 임베딩+하이브리드+(선택)리랭크 후 → 근거를 인용하며 답변 생성.
```
한국어 쿼리
...
> **인용/grounding** = 답변 문장마다 근거가 된 청크 번호를 붙여 출처를 확인 가능케 하는 것. 목적은 **환각**(LLM이 사실 아닌 내용을 그럴듯하게 지어내는 현상) 억제.
