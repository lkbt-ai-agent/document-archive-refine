...
- `processing` 좀비는 lessons/01 수정(CancelledError 종결)으로 신규 발생이 차단됐다. 과거 잔존분만 해당한다.
...
- 따라서 객체가 있는 `failed` 문서를 전체 재실행하는 것은 안전하다. lessons/02 복구가 이를 실증했다(3개 재인제스트로 ready).
...
- lessons/02 수동 복구가 성공한 이유는 결과 키가 이미 만료됐기 때문이다.
